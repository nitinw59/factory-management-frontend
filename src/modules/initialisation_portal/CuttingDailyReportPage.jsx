...
import { Loader2, Download, RefreshCw, Calendar, FileText } from 'lucide-react';
...
const CuttingDailyReportPage = () => {
    ...
    const fetchData = async () => {
        setIsLoading(true);
        try {
            const res = await cuttingPortalApi.getDailyReport();
            // Optional: Filter by date on frontend or send params to backend
            // For now, we show all, but you can filter 'data' here
            setData(res.data); 
        } catch (err) {
            console.error(err);
        } finally {
    ...
    const totals = useMemo(() => {
        return data.reduce((acc, row) => {
            ...
            acc.fab_stock += (parseFloat(row.assigned_fabric || 0) - parseFloat(row.consumed_fabric || 0) - parseFloat(row.rejection || 0));
            return acc;
        }, { 
    ...
    const handleDownloadPDF = () => {
        ...
        doc.save(`Cutting_Status_Report_${new Date().toISOString().split('T')[0]}.pdf`);
    };
        ...
        <div className="p-6 bg-gray-100 min-h-screen font-inter">
            <header className="mb-6 flex justify-between items-center bg-white p-4 rounded-xl shadow-sm border border-gray-200">
                <div>
                    ...
                    <p className="text-sm text-gray-500">Live overview of cutting production and fabric utilization.</p>
                </div>
                <div className="flex gap-3">
                    <button onClick={fetchData} className="p-2 text-gray-600 hover:bg-gray-100 rounded-lg transition-colors" title="Refresh">
                        <RefreshCw size={20} />
                    </button>
                    <button onClick={handleDownloadPDF} className="flex items-center px-4 py-2 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700 shadow-sm transition-colors">
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-xs text-left whitespace-nowrap">
                            ...
                            <tbody className="bg-yellow-100 font-bold border-b-2 border-gray-300 text-gray-900">
                                <tr>
                                    <td colSpan={6} className="py-2 px-2 text-right uppercase">Total:</td>
                                    <td className="py-2 px-2 text-right">{totals.cut_qty}</td>
                                    <td className="py-2 px-2 text-right">{totals.load_qty}</td>
                                ...
                                {data.map((row, idx) => {
                                    ...
                                    return (
                                        <tr key={idx} className="hover:bg-blue-50 transition-colors">
                                            ...
                                            <td className="py-2 px-2 text-right font-bold">{row.cut_qty}</td>
                                            <td className="py-2 px-2 text-right text-gray-600">{row.load_qty}</td>
