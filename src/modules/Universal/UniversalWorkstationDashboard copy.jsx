...
import { 
    ...
    Search, FolderCheck, CheckSquare, Square, XCircle, ArrowLeft, 
...
const StageCompletionHandoff = ({ batchId, lineId, onBatchComplete }) => {
    ...
    const handleHandoff = async () => {
        setIsLoading(true);
        try {
            // Note: Ensure `checkCompletion` is added to your universalApi.js!
                ...
                <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setWipReport(null)}>
                    <div className="bg-white rounded-3xl shadow-2xl w-full max-w-3xl overflow-hidden animate-in fade-in zoom-in-95" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="p-5 bg-white border-t border-slate-200 flex justify-end">
                            <button 
                                onClick={() => setWipReport(null)}
                                className="px-8 py-3 bg-slate-800 text-white font-bold rounded-xl hover:bg-slate-900 active:scale-95 transition-all shadow-sm"
                            >
                                Resume Scanning
                            </button>
                        </div>
                ...
                <div className="px-6 py-4 border-b bg-slate-50 flex justify-between items-center shrink-0">
                    <div>
                        <h3 className="text-xl font-black text-slate-800 tracking-tight">
                            {itemInfo.isBundle ? `Exploded Bundle: ${itemInfo.bundle_code}` : `Piece-Level QC: ${itemInfo.partName}`}
                        </h3>
                        ...
                        <div className="flex justify-between items-center animate-in fade-in slide-in-from-left-4 relative">
                            ...
                            <div className="grid grid-cols-3 gap-4 w-3/4">
                                <button 
                                    ...
                                    disabled={
                                        selectedIds.size === 0 || 
                                        (itemInfo.isBundle && selectedIds.size !== pendingPieces.length)
                                    } 
        ...
        <div className="fixed inset-0 bg-slate-900/70 z-50 flex justify-center items-center p-4 backdrop-blur-sm" onClick={!isSubmitting ? onClose : undefined}>
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
                 <div className="px-6 py-4 border-b bg-amber-50 border-amber-100 flex justify-between items-center shrink-0">
                    <div>
                        <h3 className="text-xl font-black text-amber-900 tracking-tight">Validate Repairs: {itemInfo.isBundle ? itemInfo.bundle_code : itemInfo.partName}</h3>
...
const ValidationProgressRow = ({ label, icon: Icon, entity, onInspect, onRepair }) => {
    ...
    return (
        <div className="p-4 bg-white border border-slate-200 rounded-xl shadow-sm mb-3 flex items-center justify-between hover:shadow-md transition-shadow">
            <div className="flex items-center w-1/3">
                <div className="p-2 bg-indigo-50 text-indigo-600 rounded-lg mr-3"><Icon size={20}/></div>
                <div>
                    <span className="font-black text-slate-800 tracking-tight block">{label}</span>
                    <span className="text-xs text-slate-500 font-bold">{total_processed} / {total_cut} pieces</span>
                </div>
            </div>

            <div className="w-1/3 h-2.5 bg-slate-100 rounded-full flex overflow-hidden shadow-inner mx-4">
            ...
            <div className="w-1/3 flex justify-end items-center space-x-2">
                ...
                {!isComplete ? (
                    <button onClick={() => onInspect(entity)} className="px-4 py-2 text-sm bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 font-bold shadow-sm active:scale-95 flex items-center">
                        Inspect <ChevronRight className="w-4 h-4 ml-1" />
                    </button>
                ) : (
                    <span className="px-3 py-1.5 text-xs bg-green-50 text-green-700 border border-green-200 rounded-md font-bold flex items-center w-max">
...
const UniversalWorkstationDashboard = () => {
    ...
    const applyLocalPieceUpdate = useCallback((batchId, bundleId, rollId, partId, size, pieceIds, qcStatus) => {
        setBatches(prevBatches => prevBatches.map(batch => {
            if (batch.batch_id !== batchId) return batch;
            
            if (bundleId && batch.bundles) {
                return { ...batch, bundles: batch.bundles.map(bundle => {
                    if (bundle.bundle_id !== bundleId) return bundle;
                    const updatedPieces = (bundle.pieces || []).map(p => pieceIds.includes(p.id) ? { ...p, qc_status: qcStatus } : p);
                    return { ...bundle, pieces: updatedPieces };
                })};
            }
            
            if (rollId && batch.rolls) {
                return { ...batch, rolls: batch.rolls.map(roll => {
                    ...
                    return { ...roll, parts_details: roll.parts_details.map(part => {
                        ...
                        return { ...part, size_details: part.size_details.map(sz => {
                            ...
                            const updatedPieces = (sz.pieces || []).map(p => pieceIds.includes(p.id) ? { ...p, qc_status: qcStatus } : p);
    ...
    const handleValidationSubmit = async (validationData) => {
        try {
            ...
            applyLocalPieceUpdate(validationData.batchId, validationData.bundleId, validationData.rollId, validationData.partId, validationData.size, validationData.pieceIds, validationData.qcStatus);
            setModalState(null);
    ...
    const handleApproveAlterSubmit = async (pieceIds) => {
        try {
            const { batchId, bundle_id, rollId, partId, size } = modalState;
            await universalApi.approveAlteredPieces({ batchId, pieceIds });
            applyLocalPieceUpdate(batchId, bundle_id, rollId, partId, size, pieceIds, 'REPAIRED');
            setModalState(null);
        } catch (err) { alert(err.response?.data?.error || `Error: ${err.message}`); }
    };
                                ...
                                <div className="p-6">
                                    {isBundleMode && batch.bundles && batch.bundles.length > 0 ? (
                                        batch.bundles.map(bundle => (
                                            <ValidationProgressRow 
                                                ...
                                                label={`Bundle: ${bundle.bundle_code} (${bundle.part_name})`} 
                                                icon={Package} 
                                                entity={bundle}
                                    ...
                                    {!isBundleMode && batch.rolls && batch.rolls.length > 0 ? (
                                        batch.rolls.map(roll => (
                                            <div key={roll.roll_id} className="mb-6 last:mb-0 border-l-4 border-indigo-500 pl-4 bg-indigo-50/20 py-2 rounded-r-xl">
                                                <h3 className="font-black text-indigo-900 mb-3 flex items-center text-sm">
                                                    <Layers className="w-4 h-4 mr-2 text-indigo-600"/> ROLL #{roll.roll_id}
                                                </h3>
                                                {roll.parts_details.map(part => 
                                                    part.size_details.map(size => (
                                                        <ValidationProgressRow 
                                                            key={`${part.part_id}-${size.size}`} 
                                                            label={`${part.part_name} - Size ${size.size}`} 
                                                            icon={Component} 
                                                            entity={size}
                                                            onInspect={() => setModalState({ type: 'validate', isBundle: false, batchId: batch.batch_id, batchCode: batch.batch_code, rollId: roll.roll_id, partId: part.part_id, partName: part.part_name, size: size.size, pieces: size.pieces })}
                                                            onRepair={() => setModalState({ type: 'alter', isBundle: false, batchId: batch.batch_id, batchCode: batch.batch_code, rollId: roll.roll_id, partId: part.part_id, partName: part.part_name, size: size.size, pieces: size.pieces })}
                                                        />
                                                    ))
                                                )}
                                            </div>
