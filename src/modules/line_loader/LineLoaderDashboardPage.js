...
import { LuCircle, LuLoader, LuPencil} from 'react-icons/lu';
...
const LineSelectionModal = ({ batchId, cycleFlow, currentLineId, onClose, onSave }) => {
    ...
    useEffect(() => {
        const fetchLines = async () => {
            setIsLoading(true);
            try {
                const response = await lineLoaderApi.getLinesByType(cycleFlow.line_type_id);
    ...
    return (
        <div>
            <h3 className="text-lg font-medium mb-4">Select Production Line for: <span className="font-bold">{cycleFlow.line_type_name}</span></h3>
            {isLoading ? <Spinner /> : (
            ...
            <div className="mt-6 flex justify-end space-x-3">
                ...
                <button onClick={handleSave} disabled={!selectedLine || isLoading} className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 disabled:bg-blue-300">Assign / Change</button>
            </div>
        </div>
    );
};
...
const BatchCard = ({ batch, onAssign }) => {
    ...
    const { nextStepSequence, overallStatus } = useMemo(() => {
        if (!batch.cycle_flow || batch.cycle_flow.length === 0) {
            return { nextStepSequence: -1, overallStatus: 'NO_FLOW' };
        }
        const completedProgress = batch.progress?.filter(p => p.status === 'COMPLETED') || [];
        if (completedProgress.length === batch.cycle_flow.length) {
            return { nextStepSequence: -1, overallStatus: 'COMPLETED' };
        }
        const completedFlowIds = new Set(completedProgress.map(p => p.product_cycle_flow_id));
        const highestCompletedSeq = batch.cycle_flow
            .filter(cf => completedFlowIds.has(cf.id))
            .reduce((max, cf) => Math.max(max, cf.sequence_no), 0);
        const nextSeq = highestCompletedSeq + 1;
        const currentOverallStatus = batch.progress?.length > 0 ? 'IN_PROGRESS' : 'PENDING';
        return { nextStepSequence: nextSeq, overallStatus: currentOverallStatus };
    }, [batch]);
    ...
    return (
        <div className="bg-white rounded-lg shadow-md overflow-hidden">
            <div className={`px-4 py-2 flex items-center justify-between ${currentStatus.bg}`}>
                <h2 className="font-bold text-lg">{batch.product_name} - <span className="font-mono">{batch.batch_code}</span></h2>
                <span className={`flex items-center text-sm font-semibold px-2 py-1 rounded-full ${currentStatus.bg} ${currentStatus.text}`}>
                   {currentStatus.icon && <span className="mr-2">{currentStatus.icon}</span>}
                   {currentStatus.label}
                </span>
            </div>
            <div className="p-4">
                ...
                <div className="flex flex-wrap gap-2">
                    {batch.cycle_flow?.map(cf => {
                        ...
                        const buttonStyles = {
                           COMPLETED: 'bg-green-500 text-white cursor-not-allowed',
                           PENDING: 'bg-yellow-500 text-white hover:bg-yellow-600', // Yellow for assigned, now clickable
                           ENABLED: 'bg-blue-500 text-white hover:bg-blue-600', // Blue for the next actionable step
                           DISABLED: 'bg-gray-200 text-gray-500 cursor-not-allowed'
                        };
                        
                        const isDisabled = buttonState === 'DISABLED' || buttonState === 'COMPLETED';
                        ...
                        return (
                            ...
                            >
                               {cf.line_type_name}
                               {buttonState === 'PENDING' && (
                                 <span className="ml-2 text-xs font-normal opacity-90 truncate">({progressEntry.line_name}) <LuPencil className="inline-block ml-1" size={10}/></span>
                               )}
                            </button>
                        );
                    })}
        ...
        <div>
            ...
            {isLoading ? <Spinner /> : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {batches.map(batch => (
                        <BatchCard key={batch.batch_id} batch={batch} onAssign={handleAssign} />
                    ))}
                </div>
            )}
        </div>
