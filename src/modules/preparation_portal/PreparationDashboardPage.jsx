...
import { preparationPortalApi } from '../../api/preparationPortalApi';
...
const PreparationDashboardPage = () => {
    ...
    const fetchQueue = useCallback(() => {
        setIsLoading(true);
        preparationPortalApi.getMyQueue()
            .then(res => {
                console.log("Fetched preparation queue:", res.data);
                setBatches(res.data || []);
            })
            .catch(() => setError("Could not load your preparation queue."))
    ...
    const handlePreparationSuccess = () => {
        ...
        <div className="bg-white p-4 rounded-lg shadow border-l-4 border-purple-500">
            <div className="flex justify-between items-center">
                ...
                <button onClick={() => setSelectedBatch(batch)} className="flex items-center px-3 py-1 text-sm bg-purple-600 text-white rounded hover:bg-purple-700">
                    <LuClipboardCheck className="mr-1" /> Prepare
                </button>
            </div>
    ...
    return (
        <div>
            <h1 className="text-3xl font-bold mb-6">My Preparation Queue</h1>
            {isLoading ? <Spinner /> : error ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {batches.length > 0 ? (
                        ...
                        <p className="text-center text-gray-500 p-4 col-span-full">No batches are currently assigned to your line for preparation.</p>
                    )}
                </div>
            )}

            {selectedBatch && (
                <Modal title={`Prepare Supporting Pieces for Batch #${selectedBatch.id}`} onClose={() => setSelectedBatch(null)}>
                    <preparationForm
                        batchId={selectedBatch.id}
                        onSaveSuccess={handlePreparationSuccess}
...
const PreparationForm = ({ batchId, onSaveSuccess, onClose }) => {
    ...
    useEffect(() => {
        preparationPortalApi.getPreparationDetails(batchId)
            .then(res => {
                setPieces(res.data || []);
            })
            .finally(() => setIsLoading(false));
    ...
    const handleSave = async () => {
        setIsSaving(true);
        try {
            await preparationPortalApi.preparePieces({ cutPieceLogIds: Array.from(checkedIds) });
            onSaveSuccess();
        } catch (error) {
        ...
        <div className="p-4">
            ...
            <div className="mt-6 flex justify-end space-x-3">
                ...
                <button onClick={handleSave} disabled={isSaving || checkedIds.size === 0} className="flex items-center px-6 py-2 bg-purple-600 text-white rounded-lg disabled:bg-purple-300">
                    <LuBookHeart className="mr-2"/> Prepare ({checkedIds.size}) Selected
                </button>
            </div>
        </div>
    );
...
export default PreparationDashboardPage;
